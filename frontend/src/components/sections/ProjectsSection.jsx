...
export default function ProjectsSection() {
  ...
  useEffect(() => {
    ...
    // Listen for custom real-time window update events!
  ...
  return (
    <section id="projects" className="relative bg-dark overflow-hidden py-16 sm:py-20 px-6">
      {/* Electric purple particle background */}
        ...
        {loading ? (
          ...
          <div className="projects-grid grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {currentProjects.map((project) => (
              <div 
                key={project.id}
                className="project-card bg-surface-2 border border-white/5 overflow-hidden rounded-2xl hover:border-secondary/45 hover:shadow-[0_0_20px_rgba(0,212,255,0.04)] transition-all duration-500 flex flex-col h-full group"
              >
                ...
                <div className="p-5 sm:p-6 flex flex-col flex-1 font-sans">
                  ...
                  {/* Render dynamic sub-libraries if they exist (Python details) */}
                  {project.libraries && (
        ...
        {totalPages > 1 && (
          <div className="flex justify-center items-center gap-3 mt-12 select-none font-mono">
            <button 
              ...
              className="w-9 h-9 rounded-xl border border-white/5 bg-surface-2/65 flex items-center justify-center text-gray-400 hover:text-white hover:border-secondary/40 disabled:opacity-30 disabled:hover:border-white/5 transition-all duration-300 active:scale-95 text-sm"
              title="Previous Page"
            >
              &larr;
            </button>
            <div className="flex items-center gap-2.5 px-3 py-2 bg-surface-2/40 border border-white/5 rounded-xl">
